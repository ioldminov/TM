...
Функция офОпределитьТекущегоПользователя() Экспорт
	...
	Если обЗначениеНеЗаполнено(Пользователь) Тогда
		...
		// Насибуллин И.Р. 04.07.2017 Закомментил. Ругается на отсутствие свойств.
...
Функция офПолучитьСписокПраздниковРФ(КалендарныйГод) Экспорт
	...
	ИначеЕсли КалендарныйГод < 2017 Тогда // Федеральный закон от 23.04.2012 № 35-ФЗ
		СписокПраздников.Добавить("0101", "Новогодние каникулы");
		СписокПраздников.Добавить("0102", "Новогодние каникулы");
		СписокПраздников.Добавить("0103", "Новогодние каникулы");
		СписокПраздников.Добавить("0104", "Новогодние каникулы");
		...
		СписокПраздников.Добавить("0223", "День защитника Отечества");
		СписокПраздников.Добавить("0224", "День защитника Отечества");
		
		СписокПраздников.Добавить("0308", "Международный женский день");
		
		СписокПраздников.Добавить("0501", "Праздник Весны и Труда");
		
		СписокПраздников.Добавить("0508", "День Победы");
		СписокПраздников.Добавить("0509", "День Победы");
		
		СписокПраздников.Добавить("0612", "День России");
		//СписокПраздников.Добавить("0613", "День России2");
		СписокПраздников.Добавить("1104", "День народного единства");	
	ИначеЕсли КалендарныйГод < 2018 Тогда  // Федеральный закон от 23.04.2012 № 35-ФЗ
		СписокПраздников.Добавить("0101", "Новогодние каникулы");
		СписокПраздников.Добавить("0102", "Новогодние каникулы");
		СписокПраздников.Добавить("0103", "Новогодние каникулы");
		СписокПраздников.Добавить("0104", "Новогодние каникулы");
		...
		СписокПраздников.Добавить("0612", "День России");
		//СписокПраздников.Добавить("0613", "День России2");
		СписокПраздников.Добавить("1106", "День народного единства");	
	Иначе // Федеральный закон от 23.04.2012 № 35-ФЗ
		...
		СписокПраздников.Добавить("0308", "Международный женский день");
		СписокПраздников.Добавить("0309", "Международный женский день");
		
		СписокПраздников.Добавить("0430", "Праздник Весны и Труда");
		СписокПраздников.Добавить("0501", "Праздник Весны и Труда");
		СписокПраздников.Добавить("0502", "Праздник Весны и Труда");
		
		СписокПраздников.Добавить("0509", "День Победы");
		
		СписокПраздников.Добавить("0610", "День России");
		СписокПраздников.Добавить("0611", "День России");
		СписокПраздников.Добавить("0612", "День России");
		
		СписокПраздников.Добавить("1105", "День народного единства");	
	КонецЕсли;
...
Функция офПолучитьСписокНазначенныхРабочихДней(КалендарныйГод) Экспорт

	СписокНазначенныхРабочихДней = Новый СписокЗначений();
	Если КалендарныйГод < 2018 Тогда  // Федеральный закон от 23.04.2012 № 35-ФЗ
	Иначе // Федеральный закон от 23.04.2012 № 35-ФЗ
		СписокНазначенныхРабочихДней.Добавить("0428", "Праздник Весны и Труда");
		
		СписокНазначенныхРабочихДней.Добавить("0609", "День России");
		
		СписокНазначенныхРабочихДней.Добавить("1229", "Новый год");	
	КонецЕсли;

	Возврат СписокНазначенныхРабочихДней
...
Функция офЭтоВыходнойДень(мДата) Экспорт
	...
	СписокНазначенныхРабочихДней = офПолучитьСписокНазначенныхРабочихДней(Число(Формат(мДата, "ДФ=yyyy")));
	
	ЭтоВыходнойДень = Ложь;
	Если (ДеньНедели(мДата) = 6 ИЛИ ДеньНедели(мДата) = 7) И СписокНазначенныхРабочихДней.НайтиПоЗначению(Формат(мДата, "ДФ=MMdd")) = Неопределено Тогда 
		ЭтоВыходнойДень = Истина;
	ИначеЕсли СписокПраздников.НайтиПоЗначению(Формат(мДата, "ДФ=MMdd")) <> Неопределено Тогда 
		ЭтоВыходнойДень = Истина;
	КонецЕсли;
...
Процедура офОбработатьСобытие(мОбъект, Пользователь, СобытиеИстории, Подробности = Неопределено, Период = Неопределено) Экспорт 
	...
	Если ТипЗнч(мОбъект) = Тип("ДокументСсылка.Задача") Тогда
		...
		// 3. Проверим, что событие не наложится на предыдущее по времени
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ИсторияРаботыПодробности.Период
		|ИЗ
		|	РегистрСведений.ИсторияРаботыПодробности КАК ИсторияРаботыПодробности
		|ГДЕ
		|	ИсторияРаботыПодробности.Период = &Период
		|	И ИсторияРаботыПодробности.Объект = &Объект";
		...
		// 4. Зафиксировать Событие В Подробной Истории
		офЗафиксироватьСобытиеВПодробнойИстории(мОбъект, Пользователь, СобытиеИстории, Подробности, Период);
...
Функция офПолучитьТекстHTMLЗадачи(Задача)
	...
	ТелоСтатьи 			= СокрЛП(Задача.СутьРешенияЗадачи);
	// Насибуллин И.Р. 04.07.2017 TM }
...
Функция офДобавитьЛидирующиеСимволы(Знач мНомерЗадачи, ТипДокумента = "Задача") Экспорт 
...
Функция офПолучитьЗадачуПоНомеру(Знач мНомерЗадачи) Экспорт
	...
	мНомерЗадачи = офДобавитьЛидирующиеСимволы(мНомерЗадачи);
...
Функция офИзменитьДатуНаДень(Знач ТекДата, НаправлениеВперед = Истина) Экспорт
	
	Если НаправлениеВперед = Истина Тогда 
		СмещениеДень = 86400;
	Иначе
		СмещениеДень = -86400;
	КонецЕсли;
	...
	К = 0;
	ТекДата = ТекДата + СмещениеДень;
	Пока Общий.офЭтоВыходнойДень(ТекДата) И К < 20 Цикл 
		ТекДата = ТекДата + СмещениеДень;
		К = К + 1;
	КонецЦикла;
