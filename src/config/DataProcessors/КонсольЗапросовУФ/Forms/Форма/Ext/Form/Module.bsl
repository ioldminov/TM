...
Процедура ПриОткрытии(Отказ)
	...
	// Сол{
	Если Не ПустаяСтрока(Объект.ПутьКФайлуЗапроса) Тогда 
		ВыбранноеИмя = Объект.ПутьКФайлуЗапроса;
		
		пФайл = Новый Файл(ВыбранноеИмя);
		Если пФайл.Существует() Тогда
			АдресВременногоХранилища = "";
			Оповещение = Новый ОписаниеОповещения("ОткрытьДеревоЗапросовЗавершение", ЭтаФорма, Истина);
			НачатьПомещениеФайла(Оповещение, АдресВременногоХранилища, ВыбранноеИмя, Ложь, УникальныйИдентификатор);
		Иначе 
			Сообщить("Не обнаружен сохраненный в настройке файл запросов <"+ВыбранноеИмя+">");
		КонецЕсли;
	КонецЕсли;
	// Сол}
...
Процедура ВыполнитьЗапрос(Команда)
	...
	Элементы[пИмя].ТолькоПросмотр = Истина;
...
Процедура РедактированиеТаблицы(Команда)
	...
	Элементы[пИмя].ТолькоПросмотр = НЕ Редактировать;
