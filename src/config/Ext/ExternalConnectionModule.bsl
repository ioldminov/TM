...
Функция ПолучитьОписаниеРелизаКраткое(IDРелиза) Экспорт 
	
	Результат = "";
	
	РелизКонфигурации = Справочники.РелизыКонфигураций.НайтиПоКоду(IDРелиза);
	
	Если ЗначениеЗаполнено(РелизКонфигурации) Тогда
		Результат = СокрЛП(РелизКонфигурации.ОписаниеРелиза);
	КонецЕсли;
