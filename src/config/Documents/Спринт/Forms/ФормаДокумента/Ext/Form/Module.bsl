...
Функция ЗадачаПрисутствуетВСписке(Задача, СоставСпринта)
	...
	Если НайденныеСтроки.Количество() = 1 Тогда
		//Если НайденныеСтроки[0].НомерСтроки = НомерТекущейСтроки Тогда 
		//	Результат = Истина;
		//КонецЕсли;
		Результат = Ложь;
...
&НаСервере
Функция НазначитьЗадачуИсполнителюНаСервере(Задача, Исполнитель)
	
	ЗадачаОбъект = Задача.ПолучитьОбъект();
	ЗадачаОбъект.Исполнитель = Исполнитель;
	ЗадачаОбъект.СостояниеЗадачи = ПредопределенноеЗначение("Справочник.СостоянияЗадач.Назначена");
...
&НаКлиенте
Процедура НазначитьИсполнителюПоРаспределению(Команда)
	
	Результат = НазначитьЗадачуИсполнителюНаСервере(Элементы.СоставСпринта.ТекущиеДанные.Задача, Элементы.СоставСпринта.ТекущиеДанные.ИсполнительРаспределение);
	ОбновитьРеквизитыВсехСтрокСоставаСпринта();
