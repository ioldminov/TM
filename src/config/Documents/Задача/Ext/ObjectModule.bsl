...
Процедура ПроверитьКорректностьНаСервере(ЭтаФорма, ТекущийОбъект, Отказ, МассивСообщений) Экспорт
	...
	Если ТекущийОбъект.СостояниеЗадачи = Справочники.СостоянияЗадач.Закрыта И Не ЗначениеЗаполнено(ТекущийОбъект.РелизКонфигурации) Тогда 
		МассивСообщений.Добавить(Новый Структура("ТекстСообщения, Поле", "Не указано значение поля <Релиз конфигурации>", "Объект.РелизКонфигурации"));
		Отказ = Истина;
	КонецЕсли;
