...
Процедура _ПередЗаписьюНаСервере(ЭтаФорма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	ТекущийОбъект.ХЗ_ОписаниеДляРелиза = Новый ХранилищеЗначения(ЭтаФорма.ФД_ОписаниеДляРелиза);
	ТекущийОбъект.ОписаниеДляРелиза = ЭтаФорма.ФД_ОписаниеДляРелиза.ПолучитьТекст();
...
Процедура _ПриЧтенииНаСервере(ЭтаФорма, ТекущийОбъект) Экспорт
	...
	ЭтаФорма.ФД_ОписаниеДляРелиза = ТекущийОбъект.ХЗ_ОписаниеДляРелиза.Получить();
	Если ПустаяСтрока(ЭтаФорма.ФД_ОписаниеДляРелиза.ПолучитьТекст()) И Не ПустаяСтрока(ТекущийОбъект.ОписаниеДляРелиза) Тогда
		_ВремСтр = СтрЗаменить(ТекущийОбъект.ОписаниеДляРелиза, "
		|", "<BR>");
		_ВремСтр = СтрЗаменить(_ВремСтр, " ", "&nbsp;");
		ЭтаФорма.ФД_ОписаниеДляРелиза.УстановитьHTML(_ВремСтр, Новый Структура);
	КонецЕсли;

КонецПроцедуры

Процедура _ЗаписатьПриложение(АдресВХранилище, ВыбранноеИмяФайла, Знач ИмяФайлаВТаблице = "") Экспорт
	
	Если ПустаяСтрока(ИмяФайлаВТаблице) Тогда 
		ИмяФайлаВТаблице = ВыбранноеИмяФайла;
	КонецЕсли;
	
	ЗВХ = ПолучитьИзВременногоХранилища(АдресВХранилище);
	НовСтр = Приложения.Добавить();
	НовСтр.ХЗ = Новый ХранилищеЗначения(ЗВХ);
	ОчищенноеИмяФайла = ИмяФайлаВТаблице;
	Поз = 10;
	Пока Поз > 0 Цикл
...
Процедура ПроверитьКорректностьНаСервере(ЭтаФорма, ТекущийОбъект, Отказ, МассивСообщений) Экспорт
	...
	Если ТекущийОбъект.СостояниеЗадачи = Справочники.СостоянияЗадач.Закрыта Тогда 
		Если ТекущийОбъект.ВключатьВОписаниеПоставки = Истина Тогда 
			мОписаниеДляРелизаПустое = ПустаяСтрока(СокрЛП(ЭтаФорма.ФД_ОписаниеДляРелиза.ПолучитьТекст()));
			Если мОписаниеДляРелизаПустое Тогда 
				МассивСообщений.Добавить(Новый Структура("ТекстСообщения, Поле", "Не указано значение поля <Описание решения>", "ФД_ОписаниеДляРелиза"));
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.Задача") Тогда
		// Заполнение шапки
		Модуль = ДанныеЗаполнения.Модуль;
		Ответственный = ДанныеЗаполнения.Ответственный;
		Проект = ДанныеЗаполнения.Проект;
		ДокументОснование = ДанныеЗаполнения.Ссылка;
		Статус = ДанныеЗаполнения.Статус;
		Суть = ДанныеЗаполнения.Суть;
		ТипЗадачи = ДанныеЗаполнения.ТипЗадачи;
	КонецЕсли;
	//}}__КОНСТРУКТОР_ВВОД_НА_ОСНОВАНИИ
КонецПроцедуры

Функция ПолучитьТекстHTMLЗадачи() Экспорт 
	
	Макет = ПолучитьМакет("Задача");
	
	ТекстHTMLЗадачи = Макет.ПолучитьТекст();
	
	Возврат ТекстHTMLЗадачи;
		
КонецФункции

Функция ПолучитьТекстHTMLСтиля() Экспорт 
	
	Макет = ПолучитьМакет("Стиль");
	
	ТекстHTMLСтиля = Макет.ПолучитьТекст();
