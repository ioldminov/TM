...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("ОповеститьПользователя", 15);

КонецПроцедуры

&НаСервере
Функция ПроверитьНаличиеНовыхНовостейНаСервере(НавигационнаяСсылка = Неопределено)
...
&НаСервере
Функция ПолучитьНавигационнуюСсылкуНаЭтуФорму()
	
	Возврат ПолучитьНавигационнуюСсылку(Документы.Задача.ПолучитьФорму("ФормаСписка"));
		
КонецФункции


&НаКлиенте
Процедура ОповеститьПользователя()
	
	НавигационнаяСсылка = Неопределено;
	Если ПроверитьНаличиеНовыхНовостейНаСервере(НавигационнаяСсылка) Тогда 
...
Процедура ОткрытьФормуЗадачи(Знач мНомерЗадачи)
	...
	Если Не ПустаяСтрока(мНомерЗадачи) Тогда 
		ЗадачаСсылка = ПолучитьЗадачуПоНомеру(мНомерЗадачи);
...
&НаСервере
Функция ПолучитьЗадачуПоНомеру(Знач мНомерЗадачи)
