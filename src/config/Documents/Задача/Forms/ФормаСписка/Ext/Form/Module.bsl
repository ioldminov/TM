...
&НаКлиенте
Перем ТекНомерЗадачи, УИДФЗПроверкаНовостей, ТекЗначениеЗаголовокЗадачиНуженОткликОтМеня;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    ОбновитьЗаголовокЗадачиНуженОткликОтМеня();
    ОбновитьЗаголовокЗадачиТребуютВнедрения();
...
Процедура ПриОткрытии(Отказ)
	...
	ТекЗначениеЗаголовокЗадачиНуженОткликОтМеня = 0;
	ПодключитьОбработчикОжидания("НачатьПроверкуНаличияНовыхТребованийОтклика", 60);
	ПодключитьОбработчикОжидания("НачатьПроверкуНаличияНовыхТребующихВнедрения", 60);
...
&НаКлиенте
Функция НачатьПроверкуНаличияНовыхТребованийОтклика()
...
&НаКлиенте
Функция НачатьПроверкуНаличияНовыхТребующихВнедрения()
...
&НаСервере
Процедура ОбновитьЗаголовокЗадачиНуженОткликОтМеня()
	
	Запрос = Новый Запрос;
	Запрос.Текст = СписокНуженОткликТекущийПользователь.ТекстЗапроса;
	
	Для Каждого ТекПараметр Из СписокНуженОткликТекущийПользователь.Параметры.Элементы Цикл
		Если ТекПараметр.Использование = Истина Тогда
			Запрос.УстановитьПараметр("" + ТекПараметр.Параметр, ТекПараметр.Значение);
		КонецЕсли;
	КонецЦикла;
	
	Выборка = Запрос.Выполнить().Выбрать();
	ЗаголовокЗадачиНуженОткликОтМеня = Выборка.Количество();
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьЗаголовокЗадачиТребуютВнедрения()
	
	Запрос = Новый Запрос;
	Запрос.Текст = СписокТребуютВнедрения.ТекстЗапроса;
	
	Для Каждого ТекПараметр Из СписокТребуютВнедрения.Параметры.Элементы Цикл
		Если ТекПараметр.Использование = Истина Тогда
			Запрос.УстановитьПараметр("" + ТекПараметр.Параметр, ТекПараметр.Значение);
		КонецЕсли;
	КонецЦикла;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	ЗаголовокЗадачиТребуютВнедрения = Выборка.Количество();
...
Процедура НовостиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда 
		ОбщийКлиент.ОткрытьЗадачуПоСсылке(Элемент.ТекущиеДанные.Объект);
	КонецЕсли; 
...
Процедура ОткрытьЗадачу(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда 
		ОбщийКлиент.ОткрытьЗадачуПоСсылке(ТекущиеДанные.Объект);
	КонецЕсли; 
