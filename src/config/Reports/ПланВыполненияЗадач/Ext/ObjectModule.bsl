...
Перем СоответствиеДатыИПредставленияДаты;
...
Функция ПолучитьЗаголовокЗадачи(НомерЗадачи, Суть, Статус, ДатаПлан)
	
	ОтображениеСтатуса = "";
	
	Если Статус = ПредопределенноеЗначение("Перечисление.СтатусЗадачи.Срочный") Тогда
		ОтображениеСтатуса = " ! ! ! ";
	ИначеЕсли Статус = ПредопределенноеЗначение("Перечисление.СтатусЗадачи.Высокий") Тогда
		ОтображениеСтатуса = " ! ";
	ИначеЕсли Статус = ПредопределенноеЗначение("Перечисление.СтатусЗадачи.Обычный") Тогда
		ОтображениеСтатуса = "";
	ИначеЕсли Статус = ПредопределенноеЗначение("Перечисление.СтатусЗадачи.Низкий") Тогда
		ОтображениеСтатуса = " _ ";
	ИначеЕсли Статус = ПредопределенноеЗначение("Перечисление.СтатусЗадачи.Отменена") Тогда
		ОтображениеСтатуса = " Х ";
	КонецЕсли;
	
	ЗаголовокЗадачи = СокрЛП("(№ "+Общий.офУдалитьЛидирующиеСимволы(НомерЗадачи)+ОтображениеСтатуса+") " + Суть + Символы.ПС + "Статус: " + Статус + "; Срок: " + ?(ДатаПлан = '00010101', "-", Формат(ДатаПлан, "ДФ=dd.MM.yyyy")));
...
Процедура ЗаполнитьОбластьОбъединения(ИмяСтруктуры, СтруктураЗадачи, ОбластьОбъединения, ЛинияГраницыВерхНиз, ЛинияГраницыЛевая, ЛинияГраницыПравая, ЦветФона, УзорЗаливки)
	...
	Если ТипЗнч(СтруктураЗадачи.Задача) = Тип("ДокументСсылка.Задача") Тогда
		ОбластьОбъединения.Текст = ПолучитьЗаголовокЗадачи(СтруктураЗадачи.НомерЗадачи, СтруктураЗадачи.Суть, СтруктураЗадачи.Статус, СтруктураЗадачи.ДатаПлан);
		ОбластьОбъединения.Примечание.Текст = СокрЛП(СтруктураЗадачи.Подробности);
	Иначе
...
Процедура СформироватьОтчет(ИсхОбъект, ТабДок) Экспорт
	...
	ТЗИсполнители = Запрос.Выполнить().Выгрузить();
	Исполнители = ТЗИсполнители.ВыгрузитьКолонку("Исполнитель");
	...
	//|	ГОД(ПланВыполненияЗадач.ДатаНачалаПлан)*10000 + МЕСЯЦ(ПланВыполненияЗадач.ДатаНачалаПлан)*100 + ДЕНЬ(ПланВыполненияЗадач.ДатаНачалаПлан) КАК ДатаНачалаПланПредставление,
	//|	ГОД(ПланВыполненияЗадач.ДатаОкончанияПлан)*10000 + МЕСЯЦ(ПланВыполненияЗадач.ДатаОкончанияПлан)*100 + ДЕНЬ(ПланВыполненияЗадач.ДатаОкончанияПлан) КАК ДатаОкончанияПланПредставление,
	|	ПланВыполненияЗадач.Исполнитель КАК Исполнитель,
	|	ПланВыполненияЗадач.Задача.СостояниеЗадачи КАК СостояниеЗадачи,
	|	ПланВыполненияЗадач.Задача.Подробности КАК Подробности,
	|	ПланВыполненияЗадач.Задача.ИсполнительОтклик КАК ИсполнительОтклик,
	|	ПланВыполненияЗадач.Задача.Статус КАК Статус,
	|	ПланВыполненияЗадач.Задача.ПлановаяДатаИсполнения КАК ДатаПлан
	|ИЗ
	...
	Если ОтображатьЗадачиОперативногоПлана Тогда 
		...
		|	ОперативныйПланЗадач.Задача.Номер КАК НомерЗадачи,
		|	ОперативныйПланЗадач.Порядок КАК Порядок,
		|	ОперативныйПланЗадач.Задача.Суть КАК Суть,
		|	ОперативныйПланЗадач.Задача.Подробности КАК Подробности,
		|	ОперативныйПланЗадач.Задача.Исполнитель КАК Исполнитель,
		|	ОперативныйПланЗадач.Задача.СостояниеЗадачи КАК СостояниеЗадачи,
		|	ОперативныйПланЗадач.Задача.ИсполнительОтклик КАК ИсполнительОтклик,
		|	НЕОПРЕДЕЛЕНО КАК Период,
		|	ОперативныйПланЗадач.Задача.Статус КАК Статус,
		|	ОперативныйПланЗадач.Задача.ПлановаяДатаИсполнения КАК ДатаПлан
		|ИЗ
	...
	КолРазделителей = 1;
	Для Каждого ТекСтрокаИсполнитель Из ТЗИсполнители Цикл
		
		// Вывод всего периода по дням для тек. исполнителя
		
		ОбластьИсполнитель.Параметры.Исполнитель = ТекСтрокаИсполнитель.Исполнитель;
		...
		Пока ТекДата <= ДатаОкончания Цикл
			...
			ЗаполнитьПустуюОбласть("ПустаяОбласть", Новый Структура("Период, Исполнитель", ТекДата, ТекСтрокаИсполнитель.Исполнитель), ОбластьИсполнительДень.Область("R1C1"));
			
			ОбластьИсполнительДень.Область("R2C1").Имя = ПолучитьИмяОбластиДня("ПотокБ", ИсполнительКод, ТекДата);
			ЗаполнитьПустуюОбласть("ПустаяОбласть", Новый Структура("Период, Исполнитель", ТекДата, ТекСтрокаИсполнитель.Исполнитель), ОбластьИсполнительДень.Область("R2C1"));
		...
		ОтборСтрок.Вставить("Исполнитель", ТекСтрокаИсполнитель.Исполнитель);
		...
		Если ОтображатьЗадачиОперативногоПлана Тогда 
			// Вывод данных "Оперативного плана задач"
			СтуктураПоиска = Новый Структура("Исполнитель", ТекСтрокаИсполнитель.Исполнитель);
