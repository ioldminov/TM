...
Функция ПолучитьЗаголовокЗадачи(НомерЗадачи, Суть)
	...
	Возврат СокрЛП("(№ "+Общий.фУдалитьЛидирующиеСимволы(НомерЗадачи)+") " + Суть);	
	
КонецФункции

Функция ПолучитьНовуюСтруктуруРасшифровки(ИмяСтруктуры, Период = Неопределено, Задача = Неопределено, Исполнитель = Неопределено)
