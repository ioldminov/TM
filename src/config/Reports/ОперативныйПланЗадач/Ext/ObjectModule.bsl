...
Процедура СформироватьОтчет(ТабДок) Экспорт
	...
	Если СписокПользователей.Количество() > 0 Тогда
		...
		ТекстОтборВРаботеОтработанные = " И ДокЗадача.Исполнитель В (&СписокПользователей) ";
	...
	Запрос.УстановитьПараметр("ВыводитьВРаботеИОтработанные", ВыводитьВРаботеИОтработанные);
	Запрос.УстановитьПараметр("ВыводитьНуженОтклик", ВыводитьНуженОтклик);
	...
	|	ВЫБОР
	|		КОГДА ОперативныйПланЗадач.Исполнитель = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|			ТОГДА ОперативныйПланЗадач.Задача.Исполнитель
	|		ИНАЧЕ ОперативныйПланЗадач.Исполнитель
	...
	|ГДЕ
	|	&ВыводитьНуженОтклик
	|	И ДокЗадача.СостояниеЗадачи = ЗНАЧЕНИЕ(Справочник.СостоянияЗадач.НуженОтклик)
	...
	|		ПО ДокЗадача.Ссылка = ВТОперативныйПланЗадач.Задача
	|ГДЕ
	|	&ВыводитьВРаботеИОтработанные
	|	И (ДокЗадача.СостояниеЗадачи = ЗНАЧЕНИЕ(Справочник.СостоянияЗадач.Отработана)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ТекстОтборВРаботеОтработанные", ТекстОтборВРаботеОтработанные);
	...
	//ЦветНуженОтклик = ЦветаСтиля.ЦветНуженОткликБледный;//Новый Цвет(230, 200, 230);
		...
		Для Каждого ТекКолонка Из ТаблицаЗадачКолонки Цикл
			...
			Если СтруктураЗадачи <> Неопределено Тогда
				...
				Если СтруктураЗадачи.Состояние = Справочники.СостоянияЗадач.НуженОтклик Тогда 
					...
					КонецЕсли;
					ОбластьЗадача.Параметры.Состояние = "Ожидание отклика от " + СтруктураЗадачи.ЗадачаИсполнительОтклик;
				КонецЕсли;
				...
				Если ОкрашиватьЗаголовок Тогда 
					...
					ОбластьВыводаЗадачи.ЦветФона = WebЦвета.Лосось;
				КонецЕсли;
...
Процедура СформироватьДоскуЗадач(ТабДок) Экспорт
	...
	Если ЗначениеЗаполнено(Спринт) Тогда
		ТекстОтборПоСпринту = " И ДокЗадача.Ссылка В (&СписокЗадачСпринта) ";
	...
	|	ДокЗадача.Ссылка КАК Задача,
	|	ДокЗадача.Ссылка.Представление КАК ЗадачаПредставление,
	|	ДокЗадача.Исполнитель КАК Исполнитель,
	|	ДокЗадача.Ответственный КАК Ответственный,
	|	ДокЗадача.Суть КАК Суть,
	|	ДокЗадача.Подробности КАК Подробности,
	|	ДокЗадача.СостояниеЗадачи КАК Состояние,
	|	ДокЗадача.Исполнитель КАК ЗадачаИсполнитель,
	|	ДокЗадача.ИсполнительОтклик КАК ЗадачаИсполнительОтклик,
	|	ВЫБОР
	|		КОГДА ОперативныйПланЗадач.Задача ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ПрисутствуетВОперативномПлане
	|ПОМЕСТИТЬ ВТОперативныйПланЗадач
	|ИЗ
	|	Документ.Задача КАК ДокЗадача
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОперативныйПланЗадач КАК ОперативныйПланЗадач
	|		ПО (ОперативныйПланЗадач.Задача = ДокЗадача.Ссылка)
	|ГДЕ
	|	ДокЗадача.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусЗадачи.Отменена) //ТекстОтборПоСпринту
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТОперативныйПланЗадач.ЗадачаИсполнитель,
	|	ВТОперативныйПланЗадач.ЗадачаИсполнительОтклик,
	|	ВТОперативныйПланЗадач.ПрисутствуетВОперативномПлане
	|ИЗ
	|	ВТОперативныйПланЗадач КАК ВТОперативныйПланЗадач
	|
	|УПОРЯДОЧИТЬ ПО
	...
	МассивСостояний.Добавить(Справочники.СостоянияЗадач.Приостановлена);
	...
	Пока ВыборкаСостояние.Следующий() Цикл
		...
		Пока ВыборкаЗадача.Следующий() Цикл
			СтруктураЗадачи = Новый Структура("Задача, Ответственный, Исполнитель, Суть, Подробности, Состояние, ЗадачаИсполнитель, ЗадачаИсполнительОтклик, ПрисутствуетВОперативномПлане");
			ЗаполнитьЗначенияСвойств(СтруктураЗадачи, ВыборкаЗадача);
			ТаблицаЗадач[НомерСтроки][Колонка] = СтруктураЗадачи;
			НомерСтроки = НомерСтроки + 1;
		КонецЦикла;
	...
	Для Каждого ТекСтрока Из ТаблицаЗадач Цикл
		...
		Для Каждого ТекКолонка Из ТаблицаЗадачКолонки Цикл
			...
			Если СтруктураЗадачи <> Неопределено Тогда
				...
				Если СтруктураЗадачи.Состояние = Справочники.СостоянияЗадач.НуженОтклик Тогда 
					//Если СтруктураЗадачи.Исполнитель = СтруктураЗадачи.ЗадачаИсполнительОтклик Тогда 
					Если ПараметрыСеанса.ТекущийПользователь = СтруктураЗадачи.ЗадачаИсполнительОтклик Тогда 
						ОкрашиватьЗаголовок = Истина;
					КонецЕсли;
					ОбластьЗадача.Параметры.Состояние = "Ожидание отклика от " + СтруктураЗадачи.ЗадачаИсполнительОтклик;
				КонецЕсли;
				...
				ОбластьИсполнитель = ОбластьЗадача.Область("R4C2:R4C2");
				ОбластьИсполнитель.Примечание.Текст = ?(СтруктураЗадачи.ПрисутствуетВОперативномПлане, "Присутствует в оперативном плане", "");
				ОбластьИсполнитель.Узор = ?(СтруктураЗадачи.ПрисутствуетВОперативномПлане, ТипУзораТабличногоДокумента.Узор14, ТипУзораТабличногоДокумента.БезУзора);
